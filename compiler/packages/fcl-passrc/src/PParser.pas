...
type
  ...
  public
    ...
    // Main scope parsing
    procedure ParseMain(out Module: TPasModule);
    procedure ParseUnit(out Module: TPasModule);
    ...
    procedure ParseProgram(out Module: TPasModule; SkipHeader : Boolean = False);
    procedure ParseLibrary(out Module: TPasModule);
...
function ParseSource(AEngine: TPasTreeContainer;
                     ...
                     UseStreams  : Boolean): TPasModule;
                     deprecated 'use version with options'; overload;
{$endif}
function ParseSource(AEngine: TPasTreeContainer;
                     const FPCCommandLine, OSTarget, CPUTarget: String): TPasModule;
                     deprecated 'use version with split command line';  overload;
function ParseSource(AEngine: TPasTreeContainer;
                     const FPCCommandLine, OSTarget, CPUTarget: String;
...
Var
  Args : {$IFDEF Pas2JS}TStringArray{$ELSE}TArray<string>{$ENDIF};
...
begin
  ...
  if not Assigned(FScanner.OnModeChanged) then
    FScanner.OnModeChanged := {$IFDEF Pas2JS}@{$ENDIF}OnScannerModeChanged;
...
begin
  m := {$IFDEF Pas2JS}@{$ENDIF}OnScannerModeChanged;
  if {$IFDEF Pas2JS}FScanner.OnModeChanged=m
    {$ELSE}MethodPointersEqual(FScanner.OnModeChanged, m){$ENDIF}
  then
...
procedure TPasParser.ParseMain(out Module: TPasModule);
begin
  Module:=nil;
  NextToken;
  SaveComments;
...
procedure TPasParser.ParseUnit(out Module: TPasModule);
var
  AUnitName: String;
...
procedure TPasParser.ParseProgram(out Module: TPasModule; SkipHeader : Boolean = False);
Var
  PP : TPasProgram;
...
procedure TPasParser.ParseLibrary(out Module: TPasModule);
Var
  PP : TPasLibrary;
