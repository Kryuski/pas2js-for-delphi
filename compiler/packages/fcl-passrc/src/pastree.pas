...
type
  ...
  TPasExpr = class(TPasElement)
    ...
    constructor Create(AParent : TPasElement; AKind: TPasExprKind; AOpCode: TExprOpCode); reintroduce; overload; virtual;
    destructor Destroy; override;
  end;
...
begin
  ...
  Result:=C.ClassName;
  s := Integer(Expr.Kind).ToString;
  Result:=Result+' '+s;
  s := Integer(Expr.OpCode).ToString;
  Result:=Result+' '+s;
  ...
  if C=TPrimitiveExpr then
    ...
    Result:=Result+' Value='+BoolToStr(TBoolConstExpr(Expr).Value, True)
...
begin
  if (AValue=nil) and (Parent<>nil) then
    ...
    ForEachCall({$IFDEF Pas2JS}@{$ENDIF}ClearChildReferences,nil);
    end;
