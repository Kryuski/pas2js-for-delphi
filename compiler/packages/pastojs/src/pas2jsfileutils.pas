...
{$ENDIF}
function IsASCII(const s: {$IFDEF Pas2js}string{$ELSE}RawByteString{$ENDIF}): Boolean; inline;
...
function UTF8ToConsole(const s: RawByteString): RawByteString;
function UTF16ToConsole(const s: UnicodeString): RawByteString;
...
begin
  ...
  for i:=1 to UpDirCount do
    s := s + '..'+PathDelim;
  if (FileP>FileL) and (UpDirCount>0) then
    s := s + LeftStr(s,length(s)-1)
  else
    s := s + copy(Filename,FileP);
  RelPath:=s;
  Result:=true;
end;
  ...
  begin
    repeat
      ...
      '?':
        ...
        begin
          inc(MaskP);
          c:=UTF16CharacterStrictLength(NameP);
          if c<1 then c:=1;
          inc(NameP,c);
        end else
          exit(false);
...
function IsASCII(const s: {$IFDEF Pas2js}string{$ELSE}RawByteString{$ENDIF}
  ): Boolean;
{$IFDEF Pas2js}
var
  i: Integer;
