...
type
  ...
  {$ifndef pas2js}
  TPasResHashList = TFPDictionary<TPasIdentifier>;
  {$endif}
...
begin
  ...
  if Index>=0 then
    begin
    // insert LIFO - last in, first out
    OldItem := FElevatedLocals.ValueOfIndex[Index];
    {$IFDEF VerbosePasResolver}
    if lowercase(OldItem.Identifier)<>LoName then
      raise Exception.Create('20160925183438');
    {$ENDIF}
    Item.NextSameIdentifier:=OldItem;
    FElevatedLocals.ValueOfIndex[Index] := Item;
    end
  else
    begin
    FElevatedLocals.Add(LoName, Item);
...
begin
  ...
  FElevatedLocals:=nil;
  {$ELSE}
  for var item in FElevatedLocals do
    OnClear(item.Value, nil);
...
begin
  ...
  {$ELSE}
  for var item in ElevatedLocals.FElevatedLocals do
...
begin
  ...
  if Index>=0 then
    begin
    // insert LIFO - last in, first out
    OldItem:= FExternalNames.ValueOfIndex[Index];
    {$IFDEF VerbosePasResolver}
    if OldItem.Identifier<>aName then
      raise Exception.Create('20170322235429');
    {$ENDIF}
    Item.NextSameIdentifier:=OldItem;
    FExternalNames.ValueOfIndex[Index] := Item;
    end
  else
    FExternalNames.Add(aName, Item);
  {$ENDIF}
...
begin
  ...
  Data.JSName:=JSName;
  Abort:=false;
  IterateElements(aClassName,{$IFDEF Pas2JS}@{$ENDIF}OnFindExtSystemClass,@Data,Abort);
  Result:=Data.Found;
...
begin
  ...
  {$ELSE}
  for var item in FExternalNames do
...
begin
  ...
  Abort:=false;
  IterateElements(TIName,{$IFDEF Pas2JS}@{$ENDIF}OnFindFirst,@FindData,Abort);
...
begin
  ...
  FJSBuiltInProcs[pbpDebugger]:=AddBuiltInProc(Pas2jsBuiltInProcNames[pbpDebugger],
      'procedure Debugger',
      {$IFDEF Pas2JS}@{$ENDIF}BI_Debugger_OnGetCallCompatibility,nil,
      nil,nil,bfCustom,[bipfCanBeStatement]);
  FJSBuiltInProcs[pbpAWait]:=AddBuiltInProc(Pas2jsBuiltInProcNames[pbpAWait],
      'function await(const Expr: T): T',
      {$IFDEF Pas2JS}@{$ENDIF}BI_AWait_OnGetCallCompatibility,{$IFDEF Pas2JS}@{$ENDIF}BI_AWait_OnGetCallResult,
      {$IFDEF Pas2JS}@{$ENDIF}BI_AWait_OnEval,{$IFDEF Pas2JS}@{$ENDIF}BI_AWait_OnFinishParamsExpr,bfCustom,[bipfCanBeStatement]);
end;
  ...
  if Value<>nil then
    try
      ...
      revkUnicodeString: Result:=TResEvalUTF16(Value).S;
        ...
        Result := IntToStr(Byte(Value.Kind));
...
begin
  ...
  try
    ...
    revkString:
      begin
      AddMessageStr(ClassScope.MsgStrToProc,ExprEvaluator.GetUnicodeStr(TResEvalString(Value).S,Expr),Proc);
      if EmitHints then
...
begin
  ...
  El.ForEachCall({$IFDEF Pas2JS}@{$ENDIF}OnHasAnonymousEl,@Data);
...
begin
  Data.Decl:=Decl;
  Data.El:=nil;
  Block.ForEachCall({$IFDEF Pas2JS}@{$ENDIF}OnHasElReadingDecl,@Data);
  Result:=Data.El<>nil;
end;
...
begin
  ...
  {AllowWriteln}
  for i:=0 to length(LocalVars)-1 do
    writeln('    ',i,' ',LocalVars[i].Name,': ',GetObjName(LocalVars[i].Element),' ',Byte(LocalVars[i].Kind));
  {AllowWriteln-}
end;
...
begin
  ...
  try
    ...
    if (El is TPasProgram) then
      ...
      //HasImplCode:=AddDelayedInits(Prg,Src,IntfContext); never used
      ...
      //HasImplCode:=AddDelayedInits(Lib,Src,IntfContext); never used
      ...
      if CheckPath and (Call.Expr=nil) then
        if aResolver.IsNameExpr(El.Value) then
          ...
          Call.Expr:=ConvertSubIdentExprCustom(Bin,AContext,
            {$IFDEF Pas2JS}@{$ENDIF}OnCreateReferencePathExpr,@CreateRefPathData);
          end
        else
          begin
          {$IFDEF VerbosePas2JS}
    ...
    if C=TPasUnresolvedSymbolRef then
      ...
      if Decl.CustomData is TResElDataBuiltInProc then
        ...
        case BuiltInProc.BuiltIn of
          ...
          bfCustom: begin
            s := BuiltInProc.Element.Name;
            if s = 'Debugger' then
              Result:=ConvertBuiltIn_Debugger(El,AContext)
            else if s = 'AWait' then
...
begin
  ...
  try
    ...
    if MinVal.Kind=revkInt then
      ...
      fn := Low(TPas2JSBuiltInName); // eliminate compiler warning
      MinInt := 0;
      MaxInt := 0;
      end;
  ...
  try
    ...
    if El.IsOfObject then
      ...
      if El.ClassType=TPasProcedureType then
        ...
        MethodKind := Low(TMethodKind); // eliminate compiler warning
      end;
...
var
  V: TMaxPrecInt;
begin
  Result := Low(TOrdType); // eliminate compiler warning
  if MinValue<0 then
    begin
...
begin
  ...
  try
    ...
    AssignSt.Expr:=FDS;
    //Body:=FDS.AFunction.Body; never used
...
begin
  ...
  for i:=0 to ExportSymbols.Count-1 do
    ...
    if Symb.ExportName<>nil then
      ...
      {$ifdef FPC_HAS_CPSTRING}
      revkString:
        ExpNameJS.Alias:=String(TResEvalString(EvalValue).S);
      {$endif}
      revkUnicodeString:
...
begin
  ...
  if El.ClassType=TPasRecordType then
    ...
    mt := Low(TMemberType); // eliminate compiler warning
  end;
      ...
      if InValue<>nil then
        ...
        revkUnicodeString:
          ...
          if InValue.Kind=revkString then
            EndInt:=TMaxPrecInt(length(string(TResEvalString(InValue).S)))-1
          else
          {$ENDIF}
