...
type
  ...
  private
    FElevatedLocals: TPasIdentHashList; // list of TPasIdentifier, case insensitive
...
type
  ...
  private
    ...
    FExternalNames: TPasIdentHashList; // list of TPasIdentifier, case sensitive
...
type
  ...
  public
    ...
    function IndexOfLocalVar(El: TPasElement): integer; overload;
...
type
  ...
  private
    ...
    Function CreateSubDeclNameExpr(El: TPasElement; const PasName: string;
      AContext: TConvertContext; PosEl: TPasElement = nil): TJSElement; overload;
    ...
    Function CreateIdentifierExpr(AName: string; CheckGlobal: boolean;
      PosEl: TPasElement; AContext: TConvertContext): TJSElement; overload;
  ...
  protected
    ...
    Procedure DoError(Id: TMaxPrecInt; MsgNumber: integer; const MsgPattern: string;
      const Args: array of {$IFDEF pas2js}jsvalue{$ELSE}const{$ENDIF}; El: TPasElement); overload;
    ...
    Function TransformVariableName(ErrorEl: TPasElement; Const AName: string;
      CheckGlobal: boolean; AContext : TConvertContext): string; overload; virtual;
    Function TransformVariableName(El: TPasElement; AContext : TConvertContext) : string; overload; virtual;
    ...
    Function CreateLiteralString(El: TPasElement; const s: TJSString): TJSLiteral; virtual;
...
uses
  StrUtils, FPCTypes, Generics.Collections;
...
begin
  ...
  {$ELSE}
  FElevatedLocals.TryGetValue(LoName, OldItem);
  //writeln('  Index=',Index);
...
begin
  inherited Create;
  FElevatedLocals := TPasIdentHashList.Create;
end;
...
begin
  for item in FElevatedLocals do
    OnClearElevatedLocal(item.Value, nil);
  {$IFDEF pas2js}
  FElevatedLocals := nil;
  {$ELSE}
...
begin
  FElevatedLocals.TryGetValue(lowercase(Identifier), Result)
end;
...
begin
  if (Param<>'') and (Param[1]='%') then begin
    if (length(Param)<3) or (Param[length(Param)]<>'%') then begin
      SetStr('');
      DoLog(mtWarning,nWarnIllegalCompilerDirectiveX,SWarnIllegalCompilerDirectiveX,
        ['$i '+Param]);
      exit;
    end;
    if length(Param)>255 then begin
      SetStr('');
      DoLog(mtWarning,nWarnIllegalCompilerDirectiveX,SWarnIllegalCompilerDirectiveX,
        ['$i '+copy(Param,1,255)+'...']);
      exit;
    ...
    if loParam = '%date%' then begin
      ...
      exit;
    end else if (loParam = '%pas2jstarget%') or (loParam = '%fpctargetos%') then begin
...
begin
  ...
  {$ELSE}
  FExternalNames.TryGetValue(aName, OldItem);
  //writeln('  Index=',Index);
  if OldItem <> nil then begin
    // insert LIFO - last in, first out
    {$IFDEF VerbosePasResolver}
...
begin
  ...
  curIdent := Identifier;
  while curIdent<>nil do begin
    if HasOverloadIndex(curIdent.Element) then begin
      dec(count);
...
begin
  ...
  IntfSection := nil;
  if aSection.ClassType=TImplementationSection then begin
...
begin
  ...
  for i := 0 to Declarations.Count-1 do begin
    ...
    C := El.ClassType;
    if C.InheritsFrom(TPasProcedure) then begin
      ...
      if ProcScope.DeclarationProc<>nil then
        continue
      else if Proc.IsOverride then begin
      ...
      RenameOverload(Proc);
    end else if C.InheritsFrom(TPasType) then begin
      if El.Parent is TProcedureBody then
        RenameOverload(El);
    end else if C=TPasConst then
      RenameOverload(El)
    else if C.InheritsFrom(TPasVariable) then begin
...
begin
  for i := 0 to Declarations.Count-1 do begin
    ...
    end else if (c=TPasClassType) or (c=TPasRecordType) then begin
      oldScopeCount := FOverloadScopes.Count;
      if c=TPasClassType then begin
...
begin
  ...
  if FOverloadScopes = nil then begin
...
begin
  ...
  C := El.ClassType;
  if C = TPasPointerType then begin
  ...
  okInterface:
    ...
    if El.GUIDExpr<>nil then
      ...
      try
        ...
        {$IFDEF FPC_HAS_CPSTRING}
        revkString:
          Scope.GUID := string(TResEvalString(Value).S);
        revkUnicodeString:
...
const
  ...
  begin
    ...
    for m := Low(TVariableModifier) to High(TVariableModifier) do
      if (m in El.VarModifiers) and not (m in Allowed) then begin
        {$WARN IMPLICIT_STRING_CAST OFF}
        str(Byte(m),s);
        {$WARN IMPLICIT_STRING_CAST ON}
...
begin
  FExternalNames.TryGetValue(aName, Result);
  {$IFDEF VerbosePasResolver}
  {AllowWriteln}
  if (Result<>nil) and (Result.Owner<>Self) then begin
...
begin
  ...
  for item in FExternalNames do
    OnClearHashItem(item.Value, nil);
...
begin
  ...
  Result := cIncompatible;
  if ParamResolved.BaseType=btContext then begin
...
begin
  ...
  {$ENDIF}
  FExternalNames := TPasIdentHashList.Create;
...
begin
  ...
  try
    case Value.Kind of
    {$IFDEF FPC_HAS_CPSTRING}
    revkString: Result := ExprEvaluator.GetUnicodeStr(TResEvalString(Value).S, Expr);
    {$ENDIF}
    revkUnicodeString: Result := TResEvalUTF16(Value).S;
    else
      {$WARN IMPLICIT_STRING_CAST OFF}
      str(Byte(Value.Kind), Result);
      {$WARN IMPLICIT_STRING_CAST ON}
...
begin
  ...
  try
    case Value.Kind of
    {$IFDEF FPC_HAS_CPSTRING}
    revkString: GUIDStr := ExprEvaluator.GetUnicodeStr(TResEvalString(Value).S,Expr);
    {$ENDIF}
...
begin
  ...
  if ((RightEl.ClassType=TPrimitiveExpr) or (RightEl.ClassType=TSelfExpr))
    and (RightEl.CustomData is TResolvedReference)
  then begin
...
var
  ...
  Right: TJSElement;
begin
  if LeftResolved.BaseType=btModule then begin
    // e.g. system.inttostr()
...
begin
  ...
  if (Ref.WithExprScope<>nil) and aResolver.IsTObjectFreeMethod(El) then begin
  ...
  if Decl.ClassType=TPasArgument then begin
    Result:=CreateArgumentAccess(TPasArgument(Decl),AContext,El);
    if IsImplicitCall then
      CallImplicit(Decl);
    exit;
  end;
  if Decl.ClassType=TPasProperty then begin
    ...
    try
      ...
      if CheckPath then
        if aResolver.IsNameExpr(El.Value) then
          // no special context
        else if El.Value is TBinaryExpr then begin
          // convert left
...
begin
  ...
  try
    ...
    if LeftJS is TJSPrimaryExpressionIdent then
      ...
      Obj := DotExpr.MExpr;
      DotExpr.MExpr := nil;
      Prop := CreateLiteralString(Bin.right,DotExpr.Name);
      FreeAndNil(LeftJS);
  ...
  if to_bt in btAllJSInteger then
    ...
    if ParamResolved.BaseType=btChar then
      ...
      if IsLiteralInteger(Result,Int)
        ...
        begin
        FreeAndNil(Result);
        Result := CreateLiteralString(Param,WideChar(Int));
        end
      else
        begin
...
begin
  ...
  case ResolvedEl.BaseType of
    ...
    btWideChar:
      begin
      if IsLow then
        Result := CreateLiteralString(El,#0)
      else
        Result := CreateLiteralString(El,#$ffff);
      exit;
      end;
    btByte..btIntMax:
      begin
  ...
  try
    ...
    if ThrowSt.A=nil then
      ...
      if length(El.Params)>1 then
        ...
        ThrowSt.A := CreateLiteralString(El.Params[0],'assert failed');
      end;
    Result := IfSt;
  finally
    Ref.Free;
  ...
  try
    ...
    for i := 0 to El.Elements.Count-1 do begin
      ...
      if BodySt=nil then
        ...
        if (BodySt is TJSStatementList) then begin
    ...
    if El.ElseBranch<>nil then begin
      JSCaseEl := SwitchEl.Cases.AddCase;
...
var
  ...
  begin
    ...
    try
      ...
      Scope := TPas2JSClassScope(El.CustomData);
      repeat
        if Scope.Interfaces<>nil then begin
          for i := 0 to Scope.Interfaces.Count-1 do begin
            MapsObj := nil;
            CurEl := TPasClassType(Scope.Element);
            if not IsMemberNeeded(TPasElement(CurEl.Interfaces[i])) then continue;
            if NeedIntfMap then
...
begin
  ...
  try
    ...
    for i := 0 to El.Values.Count-1 do
      ...
      ObjLit.Expr := CreateLiteralString(El,JSName);
      // add value:0
      ObjLit := Obj.Elements.AddElement;
      ObjLit.Name := JSName;
...
begin
  ...
  try
    ...
    if MinVal.Kind=revkInt then begin
      fn := pbifnRTTINewInt;
      MinInt := TResEvalInt(MinVal).Int;
      MaxInt := TResEvalInt(MaxVal).Int;
    end else if MinVal.Kind=revkEnum then begin
      fn := pbifnRTTINewEnum;
      ...
      MinInt := 0;
      MaxInt := 0;
      fn := pbifnArray_Concat; // to hide compiler warning
      {$IFDEF VerbosePas2JS}
  ...
  try
    ...
    if El.IsOfObject then
      ...
      if El.ClassType=TPasProcedureType then
        ...
        MethodKind := mkProcedure // to hide compiler warning
      end;
...
begin
  ...
  {$IFDEF FPC_HAS_CPSTRING}
  revkString:
    Result := CreateLiteralString(El, string(TResEvalString(Value).S));
  {$ENDIF}
  revkUnicodeString:
    Result := CreateLiteralString(El, TResEvalUTF16(Value).S);
  revkEnum:
...
begin
  ...
  if Assigned(Expr) then begin
    ...
    else if ExprResolved.BaseType in btAllStringAndChars then begin
      US := StrToJSString(aResolver.ComputeConstString(Expr,false,true));
      ArrLit := TJSArrayLiteral(CreateElement(TJSArrayLiteral,El));
      Result := ArrLit;
      for i := 1 to length(US) do
        ArrLit.Elements.AddElement.Expr := CreateLiteralString(Expr,US[i]);
    end else
      RaiseNotSupported(Expr,AContext,20170223133034);
  end else if length(ArrayType.Ranges)=0 then begin
    // empty dynamic array: []
    Result := TJSArrayLiteral(CreateElement(TJSArrayLiteral,El));
  end else begin
    // static array
    ...
    try
      ...
      while true do begin
        ...
        if (ElTypeResolved.LoTypeEl is TPasArrayType) then begin
          CurArrayType := TPasArrayType(ElTypeResolved.LoTypeEl);
          if length(CurArrayType.Ranges)>0 then begin
            // nested static array
            continue;
          end;
        end;
      ...
      if ElTypeResolved.LoTypeEl is TPasRecordType then begin
...
begin
  ...
  try
    ...
    if LHS is TJSDotMemberExpression then
      ...
      Call.AddArg(CreateLiteralString(PosEl,TJSDotMemberExpression(LHS).Name));
      FreeAndNil(LHS);
      Call.AddArg(RHS);
      RHS := nil;
      if SkipAddRef then
    ...
    if AssignContext.Call<>nil then
      ...
      case El.Kind of
        ...
        akDivision: T := TJSDivEqAssignStatement(CreateElement(TJSDivEqAssignStatement,El));
          ...
          RaiseNotSupported(El,AContext,20161107221807);
          T := nil;
        end;
      end;
  ...
  begin
    ...
    ltIn:
      ...
      if InValue<>nil then
        ...
        revkRangeInt,revkSetOfInt: begin
          if InValue.Kind=revkSetOfInt then begin
...
function TPasToJSConverter.CreateLiteralString(El: TPasElement;
...
begin
  ...
  try
    ...
    RightParent := nil;
    repeat
      if (Right.ClassType=TJSCallExpression) then begin
        RightParent := Right;
      ...
      end else if (Right.ClassType=TJSBracketMemberExpression) then begin
        RightParent := Right;
      ...
      else if (Right.ClassType=TJSDotMemberExpression) then begin
        RightParent := Right;
      ...
      end else if (Right.ClassType=TJSPrimaryExpressionIdent) then begin
...
begin
  ArgName := Arg.Name;
  if (CompareText(ArgName,'Self')=0) and (AContext.GetSelfContext<>nil) then
