...
type
  ...
  protected
    ...
    function ReadFile(Filename: string; var Source: TBinarySource; var Attr: TPas2jsFileSrcAttr): boolean; virtual;
...
begin
  ...
  if Binary then
  begin
    SetBinarySource(NewSource);
    FFileEncoding:=EncodingBinary;
  end else
  begin
    {$IFDEF FPC_HAS_CPSTRING}
    SetSource(ConvertTextToUTF16(NewSource,FFileEncoding));
    {$ELSE}
    SetSource(NewSource);
    {$ENDIF}
  end;
...
function TPas2jsFilesCache.ReadFile(Filename: string; var Source: TBinarySource;
  ...
  var
    ...
    begin
      ...
      for i:=0 to length(SearchedDir)-1 do
        if SearchedDir[i]=Dir then exit(True);
